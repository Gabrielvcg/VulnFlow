...
- Resolve request finding context inside the authorized service transaction so the selected image loads in production.
...
- Prepared the repository for public release with a license, security policy,
  contribution guidance, and an explicit exclusion for generated analysis
  artifacts.
- Removed internal `.codex/` work products from the release surface; historical
  references will be scrubbed before visibility changes.
