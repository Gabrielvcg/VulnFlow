            ...
            try {
                ...
                try (ScanArtifact artifact = executor.submit(() -> scanner.scan(target)).get()) {
                    ...
                    LOGGER.info("event=command_scan_completed agentId={} requestId={} target={} result=stored",
                            agentId, claim.requestId(), target.name());
                } catch (InterruptedException exception) {
                    Thread.currentThread().interrupt();
                    client.failScan(agentId, claim.requestId(), claim.claimToken(), "Scan interrupted");
                } catch (ExecutionException | java.io.IOException | RuntimeException exception) {
                    client.failScan(agentId, claim.requestId(), claim.claimToken(),
                            SafeErrors.limited("Scan execution failed"));
                    Throwable cause = exception instanceof ExecutionException && exception.getCause() != null
                            ? exception.getCause() : exception;
                    LOGGER.error("event=command_scan_failed agentId={} requestId={} result=reported errorType={} causeType={}",
                            agentId, claim.requestId(), exception.getClass().getSimpleName(),
                            cause.getClass().getSimpleName());
                }
            } catch (RuntimeException exception) {
                client.failScan(agentId, claim.requestId(), claim.claimToken(),
                        SafeErrors.limited("Scan request could not be executed"));
                LOGGER.error("event=command_scan_failed agentId={} requestId={} result=reported errorType={} causeType={}",
                        agentId, claim.requestId(), exception.getClass().getSimpleName(),
