...
The dedicated bucket from `infrastructure/bootstrap` is active. `backend.tf`
stores application state under `vulnflow/demo/terraform.tfstate` with native
`use_lockfile=true` locking and no credentials.

The optional `modules/rolesanywhere` path manages the non-AWS VPS workload
identity. It is disabled by default; the production input explicitly enables
it and adds a trust anchor, profile, backend IAM role, and inline role policy
after the certificate ceremony and plan review. The
...
Do not run plan, apply, or destroy as part of CI. Any future apply requires the
manual cost/security review and reviewed no-destroy plan in
`docs/aws-temporary-deployment-runbook.md`.
