...
test("public case study exposes recorded evidence without requesting private telemetry", async ({ page }) => {
  const calls: string[] = [];
  page.on("request", (r) => {
    if (r.url().includes("/api/")) calls.push(r.url());
  });
  await page.goto("/");
  await expect(page.getByRole("heading", { name: /Vulnerability data/i })).toBeVisible();
  await expect(page.getByText(/Historical production evidence/i)).toBeVisible();
  await expect(page.getByText("debian:11-slim", { exact: true })).toBeVisible();
  await page.locator("#findings").scrollIntoViewIfNeeded();
  await expect(page.getByText("CVE-2022-3715")).toBeVisible();
...
test("login fields remain inside the panel at wide and zoomed-out desktop dimensions", async ({ page }) => {
  ...
  const boxes = await page.evaluate(() => {
    ...
    const form = document.querySelector(".login-panel form")!.getBoundingClientRect();
    const input = document.querySelector<HTMLInputElement>('input[name="username"]')!.getBoundingClientRect();
    return {
...
test("operator can sign in and follow an approved scan", async ({ page }) => {
  const request = {
    ...
    targetName: "alpine-demo",
    requestedBy: "operator",
    agentId: "agent-1",
    status: "RUNNING",
    recoveryAttempts: 0,
    requestedAt: "2026-08-29T10:00:00Z",
    claimedAt: "2026-08-29T10:00:01Z",
    startedAt: "2026-08-29T10:00:02Z",
  };
  await page.route("**/api/ui/v1/**", async (route) => {
    ...
    if (path.endsWith("/scan-requests/agents"))
      return route.fulfill({
        json: [{ id: "agent-1", status: "IDLE", online: true }],
      });
    if (path.endsWith("/targets"))
      return route.fulfill({
        json: [{ id: request.targetId, name: "alpine-demo" }],
      });
    if (path.endsWith("/scan-requests") && route.request().method() === "POST") {
    ...
    if (path.endsWith("/scan-requests"))
      return route.fulfill({
        json: {
          content: [request],
          number: 0,
    ...
    if (path.endsWith(request.id)) return route.fulfill({ json: request });
    return route.fulfill({ status: 404, json: { message: "not mocked" } });
  });
  await page.goto("/login");
  await page.getByLabel("Username").fill("operator");
  await page.getByLabel("Password").fill("TemporaryPassword1A");
  await page.getByRole("button", { name: "Sign in securely" }).click();
  await page.goto("/app/scans");
  await page.getByLabel("Scan image", { exact: true }).selectOption(request.targetId);
  await expect(page.getByRole("heading", { name: "Scans", exact: true })).toBeVisible();
  await page.getByLabel("Scan agent", { exact: true }).selectOption("agent-1");
  await page.getByRole("button", { name: "Launch scan", exact: true }).click();
  await expect(page.getByRole("heading", { name: "alpine-demo" })).toBeVisible();
...
test("scan filters and completed result summary use the private API contract", async ({ page }) => {
  const request = {
    ...
    status: "COMPLETED",
    recoveryAttempts: 0,
    scanId: "55555555-5555-5555-5555-555555555555",
    eventId: "66666666-6666-6666-6666-666666666666",
    contentHash: "hash",
    scanner: "TRIVY",
    requestedAt: "2026-08-29T10:00:00Z",
    completedAt: "2026-08-29T10:00:42Z",
  };
  await page.route("**/api/ui/v1/**", async (route) => {
    const url = new URL(route.request().url());
    const path = url.pathname;
    ...
    if (path.endsWith("/summary"))
      return route.fulfill({
        json: {
          ...
          completedAt: "2026-08-29T10:00:42Z",
          findingCount: 190,
          severitySummary: { CRITICAL: 7, HIGH: 42, MEDIUM: 93, LOW: 48 },
        },
      });
    if (path.endsWith(request.id)) return route.fulfill({ json: request });
    if (path.endsWith("/scan-requests"))
  ...
  await page.getByLabel("Password").fill("TemporaryPassword1A");
  await page.getByRole("button", { name: "Sign in securely" }).click();
  await page.goto(`/app/scans?status=COMPLETED&targetId=${request.targetId}`);
  await expect(page.getByLabel("Filter scans by status")).toHaveValue("COMPLETED");
  await expect(page.getByRole("link", { name: "alpine-demo" })).toBeVisible();
  await page.getByRole("link", { name: "alpine-demo" }).click();
